## change the structure of conv model: conv + bn + activate + dropout
## change the calculation of reward
## change the training process into batch mode
## clear memory after each episode
## set target network
## update epsilon after 30 episodes
...
class DQNAgent:
    def __init__(self):
        ...
        self.model_bar = self._build_model()
        self.action_size = 2
        self.outdated = 0
    ...
    def _build_model(self):
        ...
        ## second block
        conv1_2 = Conv2D(32, (2, 2), strides=(1, 1))(x1_1)
        ...
        ## second block
        conv2_2 = Conv2D(32, (2, 2), strides=(1, 1))(x2_1)
    ...
    def replay(self, batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            target = reward
            if not done:
                a_max = np.argmax(self.model.predict(next_state)[0])
                next_estimated_reward = self.model_bar.predict(next_state)[0][a_max]
                target = (reward + self.gamma * next_estimated_reward)
...
if __name__ == '__main__':
    ...
    logfile = 'traffic_light_control_version2_8.txt'
    weightfile = 'traffic_light_control_version2_8/traffic_light_control_version2_'
